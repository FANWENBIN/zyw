$(function(){
	...
	// 图文消息列表
	if ($("#wechatAssetsList").length != 0){
		var list = {
			init: function(){
				list.dom = $("#wechatAssetsList");
	...
	if ($("#wechatSingleAsset").length != 0){
		...
		$.__singleArticleAssetImgChange__ = function(data){
			if (data){
				asset.item.find(".img").html("<img src='" + data + "' />");
			} else {
			...
			initItem: function(){
				if (window.__mulit && __mulit.cover && __mulit.sub){
					...
					if (__mulit.cover.coverimg){
						html += "<div class='p'><img src='" + __mulit.cover.coverimg + "' /></div>";
					} else {
